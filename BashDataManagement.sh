...
# connect to and creat a mounting point to a personnal working area
## connect
ssh user@??.??.??.???
## move to personal area
cd /path/to/user/
## check position of navigation
pwd
...
rm input.zip
## open execution permission to the user
chmod -R u+x input/
## open writing permission to the user
chmod -R u+w input/
## open reading permission to the user
chmod -R u+r input/
## have a look into the input data
ls input/
## creat a directory to store output data
mkdir output

# clean the file dataframe-unit1.tsv
## check if the file is from windows or linux (if windows "\r\n" if linux "\n")
...
## open the file dataframe-unit1-transposed.tsv and notice that the corresponding table is transposed
more input/dataframe-unit1-transposed.tsv
## transpose the file dataframe-unit1-transposed.tsv, then save it in output directory as dataframe-unit1.tsv
...
## exclude the sample without result (i.e. sample-00016) from the file dataframe-unit1.tsv, then save it in output directory as clean-dataframe-unit1.tsv
...
## replace the string 'sample' by 'SAMPLE' into the file dataframe-unit2.csv, then save it in output directory as clean-dataframe-unit2.csv
...
## sort per sample the file clean-dataframe-unit2.csv, replace the comma separators (i.e. csv) by tab separators (i.e. tsv), then save it in output directory as clean-dataframe-unit2.tsv
...
## notice that the unit3 provided tsv files with extention .unit3, combine it, and save it in output directory as dataframe-unit3-V1.tsv
more input/sample-00001.unit3
grep . input/*.unit3 > output/dataframe-unit3-V1.tsv
## notice that the colomn 1 and 2 of the file dataframe-unit3-V1.tsv are separated with double dots and replace them with tab separators
more output/dataframe-unit3-V1.tsv
sed 's@:@\t@'g output/dataframe-unit3-V1.tsv > output/dataframe-unit3-V2.tsv
## notice that the the strings 'input/' and '.init3' are useless in the sample identifiers (i.e. colomn 1) and remove them
...
## notice that the file output/dataframe-unit3-V3.tsv have multiple header, then remove them, save a single header and combine both
more output/dataframe-unit3-V3.tsv
### keep lines without the string '#ID'
grep -v '#ID' output/dataframe-unit3-V3.tsv > output/dataframe-unit3-V4.tsv
### keep the first line and replace the string 'sample-00001' by 'sample'
sed -n '1p' output/dataframe-unit3-V3.tsv | sed 's@sample-00001@sample@'g > output/dataframe-unit3-V4-header.tsv
### combine the header and dataframe
cat output/dataframe-unit3-V4-header.tsv output/dataframe-unit3-V4.tsv > output/dataframe-unit3-V5.tsv
## identify colomns of interest: sample (i.e. colomn 1 called sample), depth of coverage (i.e. colomn 11 called Median_fold) and GC% (i.e. colomn 10 called Read_GC)
...
## joint of the files clean-dataframe-unit1-noheader.tsv and clean-dataframe-unit2-noheader.tsv
